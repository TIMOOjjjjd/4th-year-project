...
Although the current design still leaves room for stronger graph validation, explicit edge weighting, and more systematic online adaptation, it already provides a solid and research-oriented methodological foundation. It is not merely a collection of implementation choices. Rather, it is a structured response to the combined challenges of temporal complexity, spatial dependence, uncertainty, and operational realism in urban taxi demand forecasting.
	...
	\begin{table}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{@{}lrrrr@{}}
				...
				& 24.5697 & 39.8456 & 1587.6757 & -- \\
				G1: Multi-scale + GNN 
				& 14.8361 & 27.0384 & 731.0762 & 39.62\% \\
				G2: Multi-scale + GNN + History Features 
				& 14.2896 & 26.7787 & 717.0977 & 41.84\% \\
				\bottomrule
			\end{tabular}%
		}
	\end{table}
	...
	The hourly results further support this conclusion. G1 improves over T4 in 23 out of 24 rolling target hours, while G2 improves over T4 in 23 out of 24 target hours. The average hourly MAE reduction is 31.03\% for G1 and 34.04\% for G2. The largest improvement occurs during the early morning hours, where the temporal-only model produces relatively large errors and the GNN correction is able to reduce them substantially. For example, at \texttt{2021-07-05 05:00:00}, the T4 MAE is 50.5888, while G1 reduces it to 11.0241 and G2 reduces it to 11.7685. This demonstrates that graph refinement is particularly effective when the temporal-only model makes large zone-level prediction errors.
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lrrrr}
			...
			& 16.2119 & \textbf{27.5629} & \textbf{759.7154} & 5.2176 \\
			GNN + prior only 
			& 16.4318 & 28.3577 & 804.1575 & 5.1142 \\
			GNN + stability only 
			& \textbf{16.1758} & 27.8334 & 774.7002 & 4.9281 \\
			GNN + neighbourhood only 
			& 16.6498 & 28.2616 & 798.7170 & 4.7534 \\
			GNN + full confidence 
			& 16.2176 & 27.8559 & 775.9533 & \textbf{4.6287} \\
			\bottomrule
		\end{tabular}%
	}
\end{table}

The results show that confidence weighting has a clearer effect on rolling stability than on average prediction accuracy. Compared with the unweighted GNN, the stability-only model achieves the lowest MAE, decreasing from 16.2119 to 16.1758. This corresponds to a small relative MAE reduction of approximately 0.22\%. However, the unweighted GNN still obtains the lowest RMSE and MSE, with values of 27.5629 and 759.7154 respectively. Therefore, confidence weighting should not be interpreted as uniformly improving all accuracy metrics. Instead, its main benefit is reflected in the reduction of hourly error variability.
...
The stability-only setting provides the best average MAE among the confidence-weighted variants. This result is consistent with the purpose of MC Dropout uncertainty estimation. A zone prediction with lower MC Dropout variance is more stable under stochastic forward passes, suggesting that the base temporal model is more confident about that prediction. When this stability score is used as a GNN sample weight, the residual refinement model gives more importance to reliable nodes and reduces the influence of uncertain predictions. However, the increase in RMSE and MSE compared with the unweighted model suggests that stability weighting does not necessarily reduce larger squared errors.

The prior-only setting does not improve average prediction accuracy. Its MAE increases from 16.2119 to 16.4318, while RMSE and MSE also increase to 28.3577 and 804.1575. This suggests that historical data sufficiency alone is not a sufficient indicator of prediction reliability. A zone with more historical observations may still exhibit strong short-term volatility, and high-demand zones can still produce large forecasting errors. Nevertheless, the prior-only setting reduces the standard deviation of hourly MAE from 5.2176 to 5.1142, indicating a small stability benefit.

The neighbourhood-only setting produces the weakest average MAE among the five variants, increasing MAE to 16.6498. However, it substantially improves rolling stability, reducing the standard deviation of hourly MAE from 5.2176 to 4.7534. This supports the idea that neighbourhood consistency is more useful for stabilising graph refinement than for directly reducing average error. If a zone prediction strongly disagrees with its graph neighbours, it may be less reliable as a training signal for residual correction. Down-weighting such nodes can therefore reduce hour-to-hour fluctuation, even if it does not always improve the overall MAE.

The full confidence model combines prior, stability, and neighbourhood consistency. Its MAE is 16.2176, which is very close to the unweighted GNN but slightly higher. Its RMSE and MSE are also higher than the unweighted model. However, it achieves the lowest standard deviation of hourly MAE, decreasing from 5.2176 to 4.6287. This corresponds to an improvement of approximately 11.29\% in rolling stability. Therefore, the full confidence strategy should be interpreted primarily as a stability-oriented weighting scheme rather than as the strongest configuration for average accuracy.

The hourly comparison further supports this interpretation. Across the 24 rolling target hours, the stability-only model achieves the lowest hourly MAE in 10 hours, which is more frequent than any other variant. The neighbourhood-only model achieves the best hourly MAE in 5 hours, the full confidence model and the unweighted GNN each achieve the best hourly MAE in 4 hours, and the prior-only model achieves the best hourly MAE in 1 hour. This shows that the stability score is the most consistently effective individual signal for improving hourly MAE, while the full confidence score is more effective at reducing overall hourly error variation.

Overall, Experiment 3 shows that the confidence module provides a robustness and stability enhancement rather than a dramatic accuracy improvement. The large performance gain in the overall pipeline mainly comes from the graph-based residual refinement demonstrated in Experiment 2. Confidence weighting further refines this process by controlling how much each node contributes to GNN training. Among the confidence components, MC Dropout stability is the most useful for reducing MAE, while the full confidence combination is the most useful for improving rolling stability.

The key conclusion from this experiment is therefore:
\[
\text{Stability-only: lowest MAE,}
\]
\[
\text{Unweighted GNN: lowest RMSE and MSE,}
\]
\[
\text{Full confidence: best rolling stability.}
\]

This indicates that confidence weighting is useful, but its role should be described as improving robustness and temporal stability rather than as the primary source of overall error reduction.
