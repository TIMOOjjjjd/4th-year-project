...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}lrrrr@{}}
			...
			& 33.4804 & 51.9800 & 2701.9194 & -- \\
			G1: Multi-scale + GNN 
			& 16.2106 & 30.5118 & 930.9674 & 51.58\% \\
			G2: Multi-scale + GNN + History Features 
			& 16.2040 & 30.3489 & 921.0562 & 51.60\% \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lrrrr}
			\toprule
			\textbf{Model} & \textbf{MAE $\downarrow$} & \textbf{RMSE $\downarrow$} & \textbf{MSE $\downarrow$} & \textbf{Std. of Hourly MAE $\downarrow$} \\
			\midrule
			GNN without confidence & 16.6460 & 29.2035 & 852.8439 & 7.6376 \\
			GNN + prior only & 16.7136 & 29.1922 & 852.1853 & 7.5131 \\
			GNN + stability only & \textbf{16.4645} & \textbf{28.9847} & \textbf{840.1146} & 7.5475 \\
			GNN + neighbourhood only & 16.5484 & 29.0987 & 846.7342 & \textbf{7.4499} \\
	...
	Overall, Experiment 3 shows that the confidence module provides a robustness enhancement rather than a dramatic accuracy improvement. The large performance gain in the overall pipeline mainly comes from the graph-based residual refinement demonstrated in Experiment 2. Confidence weighting further refines this process by controlling how much each node contributes to GNN training. Among the confidence components, MC Dropout stability is the most useful for reducing average error, while neighbourhood consistency is the most useful for improving rolling stability. The full confidence model provides a compromise between these effects.
	
	The key conclusion from this experiment is therefore:
	\[
	\text{Stability-only: best average accuracy,}
	\]
	\[
	\text{Neighbourhood-only: best rolling stability,}
	\]
	\[
	\text{Full confidence: balanced improvement over unweighted GNN.}
	\]
