"""Persistent pure-TCN baseline with train-once/reuse behavior."""
...
class Chomp1d(nn.Module):
    """Remove right-side padding to keep TCN convolutions causal."""
...
class TemporalBlock(nn.Module):
    """Two-layer residual causal convolution block."""
...
class PureTCN(nn.Module):
    """Temporal Convolutional Network forecaster for hourly taxi demand."""
    ...
    def __init__(
        ...
        for layer_idx in range(num_layers):
            in_channels = 1 if layer_idx == 0 else hidden_size
    ...
    def forward(self, values: torch.Tensor) -> torch.Tensor:
        ...
        features = self.tcn(values)
        last_step = self.dropout(features[:, :, -1])
...
class PureTCNModelManager:
    """Per-zone persistent TCN manager using train-once/reuse behavior."""

    def __init__(
        self,
        checkpoint_dir: str = "checkpoints_tcn",
    ...
    def _model_path(self, zone_id: int) -> Path:
        return self.checkpoint_dir / f"tcn_zone_{int(zone_id)}.pt"

    def _scaler_path(self, zone_id: int) -> Path:
        return self.checkpoint_dir / f"tcn_scaler_zone_{int(zone_id)}.pkl"

    def _meta_path(self, zone_id: int) -> Path:
        return self.checkpoint_dir / f"tcn_meta_zone_{int(zone_id)}.json"
    ...
    def _load(self, zone_id: int) -> Tuple[PureTCN, MinMaxScaler]:
    ...
    def _build_training_arrays(
        ...
        if not input_values:
            raise ValueError("Insufficient history to build TCN training windows.")
    ...
    def _build_inference_window(
        ...
        if start_idx < 0:
            raise ValueError("Not enough history to assemble TCN inference window.")
    ...
    def predict(self, df: pd.DataFrame, zone_id: int, target_date: pd.Timestamp) -> float:
        if not self.has_checkpoint(zone_id):
            raise FileNotFoundError(f"Zone {zone_id} has no TCN checkpoint.")
    ...
    def predict_with_uncertainty(
        ...
        diagnostics = {"tcn_mc_variance": float(std_orig**2)}
    ...
    def train_and_predict_if_needed(
        ...
        if not self.has_checkpoint(zone_id):
            if not auto_train:
                raise FileNotFoundError(f"No TCN checkpoint for zone {zone_id}.")
            self.train_once(df, zone_id, context_end)

        return self.predict(df, zone_id, target_date)


MultiScaleModelManager = PureTCNModelManager
