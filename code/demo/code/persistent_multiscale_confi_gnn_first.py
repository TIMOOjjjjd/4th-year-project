"""
Confidence-weighted multiscale trainer (full runs only).

Applies the MC Dropout confidence logic from multiscale_confidence without hourly
...
class MultiScaleModel(nn.Module):
    ...
    def __init__(self, hidden_size: int, p_drop: float = 0.2, spatial_dim: int = 0):
        super().__init__()
        ...
        self.gru = nn.GRU(hidden_size + 1 + spatial_dim, hidden_size, batch_first=True)
...
class MultiScaleModelManager:
    ...
    - Existing checkpoints are reused as-is; incremental updates are disabled.
    """
    ...
    def _load(self, zone_id: int) -> Tuple[nn.Module, MinMaxScaler]:
        model = MultiScaleModel(
            self.cfg.hidden_size, p_drop=self.cfg.p_drop, spatial_dim=self.cfg.spatial_dim
        ).to(device)
    ...
    def _build_training_windows(
        ...
        spatial_feat: Optional[np.ndarray] = None,
    ):
        ...
        X_tensor = self._append_spatial_feature(X_tensor, spatial_feat)
    ...
    def _build_inference_window(
        ...
        return self._append_spatial_feature(X, spatial_feat)

    @staticmethod
    def _append_spatial_feature(
        X_tensor: Dict[str, torch.Tensor], spatial_feat: Optional[np.ndarray]
    ) -> Dict[str, torch.Tensor]:
        if spatial_feat is None:
            return X_tensor
        spatial = torch.tensor(spatial_feat, dtype=torch.float32, device=device).view(1, 1, -1)
        batch = X_tensor["1h"].shape[0]
        length = X_tensor["1h"].shape[1]
        spatial_expanded = spatial.repeat(batch, length, 1)
        X_tensor["1h"] = torch.cat([X_tensor["1h"], spatial_expanded], dim=2)
        return X_tensor
    ...
    def train_once(
        ...
        X_tensor, y_tensor, scaler = self._build_training_windows(hourly, spatial_feat=spatial_feat)
    ...
    def predict(
        ...
        X_last = self._build_inference_window(hourly, scaler, context_end, spatial_feat=spatial_feat)
    ...
    def train_and_predict_if_needed(
        ...
        if not self.has_checkpoint(zone_id):
            ...
            self.train_once(df, zone_id, context_end, spatial_feat=spatial_feat)
        else:
            ...
            if prev is None:
                ...
                warnings.warn(
                    f"Incremental updates are disabled; using checkpoint trained through {prev}.",
                    RuntimeWarning,
                )

        return self.predict(df, zone_id, target_date, spatial_feat=spatial_feat)
